...
const ServiceApprovalPage = ({ serviceId }) => {
  ...
  // Default service data (for new accounts awaiting approval)
  ...
  const handleApprove = () => {
    try {
      ...
      if (serviceIndex !== -1) {
        ...
        services[serviceIndex].isApproved = true;
        localStorage.setItem('services', JSON.stringify(services));

        setService({ ...service, status: 'Active', isApproved: true });
        toast.success('Service approved successfully!');
        setIsUpdating(false);
        
        // Navigate back to services page after a short delay
        setTimeout(() => {
