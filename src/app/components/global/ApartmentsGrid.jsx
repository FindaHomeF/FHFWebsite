...
const ApartmentsGrid = ({ 
  ...
  const getFilteredItems = () => {
    ...
    // Filter out apartments with zero inventory
      ...
      <div className="w-[90%] md:w-5/6 mx-auto mt-6 md:mt-10 mb-10 md:mb-16">
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-xl text-gray-500">No properties found matching your filters.</p>
            <Button 
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 md:gap-6">
              {currentItems.map((item) => (
                <div key={item.id}>
                  <ListingBox image={item.image} propertyId={item.propertyId} item={item} />
                </div>
              ))}
            </div>
