...
export default function ArtisanSidebar({ isOpen, onClose }) {
  const { isNavItemActive, canManageServices, isNinUploaded, isNinApproved } = useArtisan();
        ...
        <nav className="flex-1 flex flex-col overflow-y-auto pt-7 px-2 z-[9999] bg-white">
          ...
          <ul className="space-y-1 px-2 flex-1">
            {navigation.map((item) => {
              ...
              const isDisabled = item.requiresApproval && !canManageServices;
              ...
              return (
                <li key={item.name}>
                  {isDisabled ? (
                    <div className={`${linkClasses} ${disabledClasses}`} title={!isNinUploaded ? "Complete profile and upload NIN" : !isNinApproved ? "NIN pending approval" : "Profile incomplete"}>
          ...
          <div className="flex-shrink-0 mt-auto p-4">
            <div className="flex items-center px-3 py-2">
              ...
              <span className="ml-3 text-sm font-medium text-tertiary">Artisan</span>
            </div>
          </div>
        </nav>
      </div>
